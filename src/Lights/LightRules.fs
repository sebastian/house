...
module private Internals =
    ...
    let appropriateBrightness () =
        ...
        |> function
            ...
            | now when now > TimeSpan (20, 00, 00) -> scaleDown 200. 100. (secondsInWindow 20 22) (secondsUntil 22) // When winding down
            | now when now > TimeSpan (17, 00, 00) -> scaleDown 254. 200. (secondsInWindow 17 20) (secondsUntil 20) // When winding down
            | now when now > TimeSpan (10, 00, 00) -> 254
            | now when now > TimeSpan (6, 15, 00) -> scaleUp 200. 254. (secondsInWindow 6 10 - 60 * 15) (secondsUntil 10) // When winding down
            | now when now > TimeSpan (6, 00, 00) -> 200
            | _ -> 10 // Before 6:00

    // The supported range is 153 (6500K) to 500 (2000K)
    ...
    let appropriateLightSettings room =
        On (appropriateBrightness (), appropriateCt room)
    ...
    let planForRooms (rooms: Room list): (Room * LightState) list =
        let primaryRoomState
            ...
            |> List.fold (fun settings room ->
                ...
                then Map.add room (appropriateLightSettings room) settings
                else Map.add room Off settings) Map.empty
        primaryRoomState
        |> Map.map (fun room currentState ->
            ...
            | Off ->
                ...
                |> function
                    | [] -> Off // All neighbouring rooms are off
                    | _neighbouringRooms -> if allowedOnAsSecondary () then appropriateLightSettings room else Off
            | On _ -> currentState)
        |> Map.toList
        ...
        |> List.filter (fun (room, _newState) ->
            ...
            | Some Off, Some lightLevel -> lightLevel < 15000
            | _ -> true
        )
